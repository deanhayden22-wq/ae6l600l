...
| Thunks resolved | 64/378 (tuning-critical all resolved; 314 unresolved are infrastructure) |
...
| `maps/ram_reference.txt` | 858 significant RAM addresses (497 named, top 200 at 100%) |
| `maps/ram_map_raw.txt` | 4,456 raw literal pool RAM refs (pre-dedup scan) |
...
| `maps/thunk_resolution.txt` | 378 thunks found, 64 resolved (tuning-critical covered) |
| `maps/desc_func_xref.txt` | 995 descriptors mapped to functions (zero orphans) |
...
### Phase 5: Quality & consistency -- COMPLETE
- ~~**Standardize analysis file format**~~ -- Done. All 62 analysis files use consistent 3-line banner header (title + ROM/ECU), `====` major sections, `--------` subsections. Removed `####`, `══`, stale file paths, `Generated by` lines.
- ~~**CL/OL FUN_ label cleanup**~~ -- Done. 160 FUN_ references replaced with canonical names (clol_transition_core, ol_condition_checker, etc.) across 5 CL/OL analysis files.
- ~~**Mystery subs label propagation**~~ -- Done. Resolved names from mystery_subs_analysis.txt propagated to accel_enrichment, map_switching, and ram_reference cross-references.
- ~~**RAM reference stats correction**~~ -- Done. Header updated: 858 significant addresses, 497 named, all >= 5 refs named. Top 200 at 100% coverage.
