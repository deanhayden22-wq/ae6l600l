#!/usr/bin/env python3
"""
Resolve mystery subroutines:
  0x29858 -- called by 4 tasks (11, 18, 25, 29)
  0x2F8FE -- called by 3 tasks (8, 10, 11)
  0x2999C -- called by 3 tasks (19, 23, 56)
Also traces sub-functions they call.
"""
...
def flt(a):
    try: return struct.unpack('>f', rom[a:a+4])[0]
    except: return None
...
KNOWN_RAM = {
    ...
    0xFFFF7448: "cl_ol_flag",
    0xFFFF65C5: "engine_running_flag",
    0xFFFF65F6: "cl_active_flag",
    0xFFFF726C: "transient_flag",
    0xFFFF7C68: "engine_status_byte",
    0xFFFF80FC: "knock_det_GBR_base",
    0xFFFF8290: "flkc_fg_GBR_base",
    0xFFFF8258: "flkc_fg_limit",
    0xFFFF3244: "flkc_fg_R0_init",
    0xFFFF3248: "flkc_fg_var",
    0xFFFF8233: "flkc_fg_flag",
    0xFFFF3360: "flkc_output_table",
    0xFFFF81F0: "knock_GBR_base",
    0xFFFF81F8: "knock_thresh_var",
    0xFFFF8210: "knock_window_var",
    0xFFFF8224: "knock_det_counter",
    0xFFFF8244: "knock_per_cyl_a",
    0xFFFF826C: "knock_timing_var",
    0xFFFF8268: "knock_win_update",
    0xFFFF829E: "flkc_win_flag",
    0xFFFF8298: "flkc_cyl_index",
    0xFFFF65BD: "cl_ol_transition_flag",
...
KNOWN_FUNCS = {
    0x022F92: "check_cl_active",
    0x022CF4: "check_engine_running",
    0x02F8EA: "check_transient_flag",
    0x03AB20: "check_engine_status",
    0x0582AC: "check_diag_state",
    0x0582D2: "check_engine_running_diag",
    ...
    0x0BE830: "table_1d_desc",
    0x0BDC52: "table_desc_1d_float",
    0x0BDC82: "table_desc_1d_uint16",
    0x0BDC9A: "table_desc_2d_typed",
    0x0BDCB6: "desc_read_int_safe",
    0x0BD8E8: "float_min",
    0x0BD8F2: "float_max",
    0x0BD900: "float_clamp_range",
    0x0BD914: "float_lerp",
    0x0BDB38: "float_deadband_check",
    0x0BDB54: "float_safe_div",
    0x0BD8E0: "float_abs_diff",
    0x0BE57E: "float_max_sat",
    0x0BA84:  "critical_section_enter",
    0x0BAC0:  "critical_section_exit",
    0x06B5A:  "calc_table_offset",
    0x000317C: "init_block_func",
    0x0003190: "init_block_func_b",
    0x00E628:  "sched_table_main",
...
def disasm_range(start, end, label=""):
    ...
    while addr < end and addr + 1 < len(rom):
        ...
        if op == 0x0009: mnem = "nop"
            ...
            elif sub == 0x9: mnem = f"movt   {rn(n)}"
            elif sub == 0xF: mnem = f"mac.l  @{rn(m)}+,@{rn(n)}+"
        elif top == 0x1:
            mnem = f"mov.l  {rn(m)},@({nib[3]*4},{rn(n)})"
        elif top == 0x2:
            sub = nib[3]
            ops2 = {0: f"mov.b  {rn(m)},@{rn(n)}", 1: f"mov.w  {rn(m)},@{rn(n)}",
            ...
            sub = nib[3]
            ops3 = {0: f"cmp/eq {rn(m)},{rn(n)}", 2: f"cmp/hs {rn(m)},{rn(n)}",
            ...
            sub = (nib[2] << 4) | nib[3]
            ops4 = {0x00: f"shll   {rn(n)}", 0x01: f"shlr   {rn(n)}",
                    ...
                    0x2E: f"ldc    {rn(n)},VBR", 0x0A: f"lds    {rn(n)},MACH",
                    0x2A: f"lds    {rn(n)},PR",
                    ...
                    0x07: f"ldc.l  @{rn(n)}+,SR", 0x17: f"ldc.l  @{rn(n)}+,GBR",
                    0x27: f"ldc.l  @{rn(n)}+,VBR",
            ...
            mnem = f"mov.l  @({nib[3]*4},{rn(m)}),{rn(n)}"
        elif top == 0x6:
            sub = nib[3]
            ops6 = {0: f"mov.b  @{rn(m)},{rn(n)}", 1: f"mov.w  @{rn(m)},{rn(n)}",
            ...
            elif sub == 0xE: mnem = f"fmac   FR0,{frn(m)},{frn(n)}"
            elif sub == 0xD:
                ss = nib[2]
...
# Show callers from task_call_graph for context
print("""
Callers from task_call_graph.txt:
  0x29858 -> tasks 11, 18, 25, 29
  0x2F8FE -> tasks  8, 10, 11
  0x2999C -> tasks 19, 23, 56
""")

# ── Disassemble each mystery sub (generous range, will hit data) ─────────
pl(disasm_range(0x29858, 0x29A00, "sub_29858 @ 0x29858"))
pl(disasm_range(0x2F8FE, 0x2FA80, "sub_2F8FE @ 0x2F8FE"))
pl(disasm_range(0x2999C, 0x29B00, "sub_2999C @ 0x2999C"))

# Also show the BSR/JSR patterns from tasks that call them to understand args
print("\n" + "="*78)
print("  CALL SITES -- context for argument passing")
print("="*78)

# task11 calls both 0x29858 and 0x2F8FE -- best context
pl(disasm_range(0x4438C, 0x44480, "task11_knock_flag (calls 0x29858 and 0x2F8FE)"))
# task29 calls 0x29858
pl(disasm_range(0x44296, 0x44330, "task29_timing_percyl (calls 0x29858)"))
# task8 calls 0x2F8FE
pl(disasm_range(0x44E04, 0x44F00, "task08_knock_window (calls 0x2F8FE)"))
# task19 calls 0x2999C
pl(disasm_range(0x45E96, 0x45F20, "task19_flkc_post (calls 0x2999C)"))
# task23 calls 0x2999C
pl(disasm_range(0x467F4, 0x46880, "task23_knock_cyl_track (calls 0x2999C)"))
